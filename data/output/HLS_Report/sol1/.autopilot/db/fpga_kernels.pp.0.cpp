...
__attribute__((sdx_kernel("predict_kernel", 0))) void predict_kernel(
    const data_t p_x[4],
...
#line 137 "/home/zzh/minkowski_fpga/src/hls/run_hls.tcl"
#pragma HLSDIRECTIVE TOP name=predict_kernel
# 17 "/home/zzh/minkowski_fpga/src/hls/fpga_kernels.cpp"



    ({ bool _AssertPred = m_x >= 0 && m_x <= 32; __builtin_assume(_AssertPred); });
    ({ bool _AssertPred = m_w >= 0 && m_w <= 32; __builtin_assume(_AssertPred); });
    ({ bool _AssertPred = m_x + m_w <= 32; __builtin_assume(_AssertPred); });
...
#pragma HLS ARRAY_PARTITION variable=H_pred complete dim=1
    ...
    VITIS_LOOP_35_1: for (i = 0; i < 4; ++i) {
#pragma HLS PIPELINE II=1
        p_pred[i] = p_w[i];
        VITIS_LOOP_38_2: for (j = 0; j < 4; ++j) {
#pragma HLS UNROLL
    ...
    VITIS_LOOP_45_3: for (k = 0; k < m_x; ++k) {

#pragma HLS LOOP_TRIPCOUNT min=0 max=32
        VITIS_LOOP_48_4: for (i = 0; i < 4; ++i) {
#pragma HLS PIPELINE II=1
            H_pred[i][k] = 0.0;
            VITIS_LOOP_51_5: for (j = 0; j < 4; ++j) {
#pragma HLS UNROLL
    ...
    VITIS_LOOP_59_6: for (k = 0; k < m_w; ++k) {
#pragma HLS LOOP_TRIPCOUNT min=0 max=32
        VITIS_LOOP_61_7: for (i = 0; i < 4; ++i) {
#pragma HLS PIPELINE II=1
...
void strip_update_kernel(
    ...
    data_t residual = 0.0;
    VITIS_LOOP_86_1: for (i = 0; i < 4; ++i) {
        residual += c[i] * p[i];
    }
    data_t r = y - residual;


    VITIS_LOOP_92_2: for (i = 0; i < 4; ++i) {
        p_hat[i] = p[i] + lambda[i] * r;
    }


    VITIS_LOOP_97_3: for (j = 0; j < m; ++j) {
        data_t t = 0.0;
        VITIS_LOOP_99_4: for (i = 0; i < 4; ++i) {
            t += c[i] * H[i][j];
        }
        VITIS_LOOP_102_5: for (i = 0; i < 4; ++i) {
            H_hat[i][j] = H[i][j] - lambda[i] * t;
        }
    }


    VITIS_LOOP_108_6: for (i = 0; i < 4; ++i) {
...
void row_sum_abs_kernel(
    ...
    VITIS_LOOP_122_1: for (int i = 0; i < 4; ++i) row_sum[i] = 0.0;

    VITIS_LOOP_124_2: for (int j = 0; j < m_drop; ++j) {
        VITIS_LOOP_125_3: for (int i = 0; i < 4; ++i) {
            data_t v = H_drop[i][j];
            row_sum[i] += (v >= 0.0) ? v : -v;
        }
    }
